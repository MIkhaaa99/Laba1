...
import java.util.*;
    ...
    public static String decodeByFrequencyAnalysis(String encodeText, String sourceText) {
        ...
        String[] bigramArray1 = (String[]) bigramMapOfEncodeText.keySet().toArray(new String[map.size()]); //зашифров текст
        String[] bigramArray2 = (String[]) bigramMapOfSourceText.keySet().toArray(new String[map.size()]); //исходный текст
        for(int i=9; i>=0; i--) { //прогон по каждой биграмме
            for(int j=0; j<2; j++) { //первый и второй символ биграммы
                String encodeSymbolOfBigram = String.valueOf(bigramArray1[i].charAt(j)); //первый символ биграммы в зашифр тексте
                String currentSymbolOfBigram = String.valueOf(bigramArray2[i].charAt(j)); //первый символ биграммы в исходном тексте

                String valueOfMonogramMap = mapReplacementLetter.get(encodeSymbolOfBigram);
                mapReplacementLetter.put(encodeSymbolOfBigram, currentSymbolOfBigram);

                for(Map.Entry<String, String> entry : mapReplacementLetter.entrySet()) {
                    if(!entry.getKey().equals(encodeSymbolOfBigram) && entry.getValue().equals(currentSymbolOfBigram)) {
                        mapReplacementLetter.put(entry.getKey(), valueOfMonogramMap);
                    }
                }
            }
        }
